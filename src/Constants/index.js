import { BsFacebook, BsInstagram, BsLinkedin, BsTwitter } from "react-icons/bs";

export const categoryLinks = [
    { id: "action", title: "Action" },
    { id: "adventure", title: "Adventure" },
    { id: "animation", title: "Animation" },
];
export const socialIcons = [
    { id: "instagram", icon: <BsInstagram />, url: "instagram.com" },
    { id: "facebook", icon: <BsFacebook />, url: "facebook.com" },
    { id: "twitter", icon: <BsTwitter />, url: "twitter.com" },
    { id: "linkedin", icon: <BsLinkedin />, url: "linkedin.com" },
];

export const mainBanner = {
    id: 1,
    title: "SUPERCHARGED FOR PROS",
    subTitle: "iPad S13+ Pro.",
    price: "from $999*",
    image: "images/main-banner-1.jpg",
};

export const smallBanner = [
    {
        id: 1,
        title: "BEST SALE",
        subTitle: "Laptops Max",
        price: "from $1699.00*",
        image: "images/catbanner-01.jpg",
    },
    {
        id: 2,
        title: "NEW ARRIVAL",
        subTitle: "Buy iPad Air",
        price: "from $599*",
        image: "images/catbanner-03.jpg",
    },
    {
        id: 3,
        title: "15% OFF",
        subTitle: "Smartwatch 7",
        price: "Shop the latest band styles & colors.",
        image: "images/catbanner-02.jpg",
    },
    {
        id: 4,
        title: "FREE ENGRAVING",
        subTitle: "Airpods Max",
        price: "High fidelity playback & ultra low distortion",
        image: "images/catbanner-04.jpg",
    },
];

export const services = [
    {
        id: 1,
        title: "Free Shipping",
        description: "Free shipping world wide",
        icon: "images/service.png",
    },
    {
        id: 2,
        title: "Surprise Offers",
        description: " New online special festival offer",
        icon: "images/service-02.png",
    },
    {
        id: 3,
        title: "12 X 7 Service",
        description: "Online Service for new Customer",
        icon: "images/service-03.png",
    },
    {
        id: 4,
        title: "Affordable Prices",
        description: "Get great prices",
        icon: "images/service-04.png",
    },

    {
        id: 5,
        title: "Secure Payments",
        description: "100% Protected Payments",
        icon: "images/service-05.png",
    },
];

export const categories = [
    {
        id: 1,
        title: "Computers & Laptops",
        image: "images/laptop.jpg",
    },
    {
        id: 2,
        title: "Camera & Videos",
        image: "images/camera.jpg",
    },
    {
        id: 3,
        title: "Smart Television",
        image: "images/tv.jpg",
    },
    {
        id: 4,
        title: "Smartwatches",
        image: "images/tv.jpg",
    },
    {
        id: 5,
        title: "Music & Gaming",
        image: "images/headphone.jpg",
    },
    {
        id: 6,
        title: "Mobile & Tablets",
        image: "images/headphone.jpg",
    },
    {
        id: 7,
        title: "Headphones",
        image: "images/headphone.jpg",
    },
    {
        id: 8,
        title: "Accessories",
        image: "images/acc.jpg",
    },
    {
        id: 9,
        title: "Portable Speaker",
        image: "images/speaker.jpg",
    },
    {
        id: 10,
        title: "Home Appliances",
        image: "images/homeapp.jpg",
    },
];

export const brandMarquee = [
    {
        id: 1,
        image: "images/brand-01.png",
    },
    {
        id: 2,
        image: "images/brand-02.png",
    },
    {
        id: 3,
        image: "images/brand-03.png",
    },
    {
        id: 4,
        image: "images/brand-04.png",
    },
    {
        id: 5,
        image: "images/brand-05.png",
    },
    {
        id: 6,
        image: "images/brand-06.png",
    },
    {
        id: 7,
        image: "images/brand-07.png",
    },
    {
        id: 8,
        image: "images/brand-08.png",
    },
];

export const blogData = [
    {
        id: 1,
        title: "The Best Headphones Of 2021: Which One Is Right For You?",
        image: "images/blog-1.jpg",
        date: "April 14, 2021",
        description:
            "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed tincidunt, erat in malesuada aliquam, est erat faucibus purus",
        link: "blog-details.html",
    },
    {
        id: 2,
        title: "The Best Headphones Of 2021: Which One Is Right For You?",
        image: "images/blog-1.jpg",
        date: "April 14, 2021",
        description:
            "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed tincidunt, erat in malesuada aliquam, est erat faucibus purus",
        link: "blog-details.html",
    },
    {
        id: 3,
        title: "The Best Headphones Of 2021: Which One Is Right For You?",
        image: "images/blog-1.jpg",
        date: "April 14, 2021",
        description:
            "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed tincidunt, erat in malesuada aliquam, est erat faucibus purus",
        link: "blog-details.html",
    },
    {
        id: 4,
        title: "The Best Headphones Of 2021: Which One Is Right For You?",
        image: "images/blog-1.jpg",
        date: "April 14, 2021",
        description:
            "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed tincidunt, erat in malesuada aliquam, est erat faucibus purus",
        link: "blog-details.html",
    },
];
